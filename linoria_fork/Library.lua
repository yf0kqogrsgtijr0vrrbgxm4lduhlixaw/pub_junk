local cloneref = (cloneref or clonereference or function(instance: any)
	...
	local Library = {
		...
		AccentColor = Color3.fromRGB(122, 158, 211),
	...
	if RunService:IsStudio() then
		...
		Library.IsMobile = (
			Library.DevicePlatform == Enum.Platform.Android or Library.DevicePlatform == Enum.Platform.IOS
		)
	end
		...
		if Library.IsMobile == false then
			Instance.InputBegan:Connect(function(Input)
				if Input.UserInputType == Enum.UserInputType.MouseButton1 then
					...
					local ObjPos =
						Vector2.new(Mouse.X - Instance.AbsolutePosition.X, Mouse.Y - Instance.AbsolutePosition.Y)
	...
	do
		...
		function BaseAddonsFuncs:AddKeyPicker(Idx, Info)
			...
			local IsInputDown = function(Input)
				...
				if SpecialKeysInput[Input.UserInputType] ~= nil then
					return InputService:IsMouseButtonPressed(Input.UserInputType)
			...
			do
				...
				function KeybindsToggle:Display(State)
					...
					Library.RegistryMap[KeybindsToggleInner].Properties.BorderColor3 = State and "AccentColorDark"
						or "OutlineColor"
					Library.RegistryMap[KeybindsToggleLabel].Properties.TextColor3 = State and "AccentColor"
						or "FontColor"
				end
			...
			function KeyPicker:GetState()
				...
				elseif KeyPicker.Mode == "Hold" then
					...
					if SpecialKeys[Key] ~= nil then
						return InputService:IsMouseButtonPressed(SpecialKeys[Key])
							and not InputService:GetFocusedTextBox()
					else
			...
			PickOuter.InputBegan:Connect(function(PickerInput)
				if
					PickerInput.UserInputType == Enum.UserInputType.MouseButton1
					and not Library:MouseIsOverOpenedFrame()
				then
					Picking = true
					...
					Picking = false
				elseif
					PickerInput.UserInputType == Enum.UserInputType.MouseButton2
					and not Library:MouseIsOverOpenedFrame()
				then
		...
		function BaseAddonsFuncs:AddColorPicker(Idx, Info)
			...
			Library:GiveSignal(InputService.InputBegan:Connect(function(Input)
				...
				if Input.UserInputType == Enum.UserInputType.MouseButton2 and ContextMenu.Container.Visible then
					if
						not Library:MouseIsOverFrame(ContextMenu.Container)
						and not Library:MouseIsOverFrame(DisplayFrame)
					then
						ContextMenu:Hide()
					end
				end
		...
		function BaseAddonsFuncs:AddDropdown(Idx, Info)
			...
			Info.FormatDisplayValue = if typeof(Info.FormatDisplayValue) == "function"
				then Info.FormatDisplayValue
				else nil
				...
				if Info.Multi and typeof(SelectedValue) == "table" then
					for Idx, Value in next, Dropdown.Values do
						...
						if SelectedValue[Value] then
							Str = Str
								.. (Info.FormatDisplayValue and tostring(Info.FormatDisplayValue(StringValue)) or StringValue)
								.. ", "
						end
					end
					...
					local StringValue = if typeof(SelectedValue) == "Instance"
						then SelectedValue.Name
						else tostring(SelectedValue)
			...
			function Dropdown:BuildDropdownList()
				...
				for Idx, Value in next, Values do
					local StringValue = if typeof(Value) == "Instance" then Value.Name else Value
					if
						Info.Searchable and not string.lower(StringValue):match(string.lower(DropdownInnerSearch.Text))
					then
						continue
					end
					...
					local ButtonLabel = Library:CreateLabel({
						...
						TextSize = 14,
						Text = Info.FormatDisplayValue and tostring(Info.FormatDisplayValue(StringValue))
							or StringValue,
						TextXAlignment = Enum.TextXAlignment.Left,
						RichText = true,
						ZIndex = 25,
	...
	do
		...
		function BaseGroupboxFuncs:AddLabel(...)
			...
			if select(2, ...) ~= nil and typeof(select(2, ...)) == "table" then
				if select(1, ...) ~= nil then
					assert(
						typeof(select(1, ...)) == "string",
						"Expected string for Idx, got " .. typeof(select(1, ...))
					)
				end
		...
		function BaseGroupboxFuncs:AddButton(...)
			...
			function Button:AddButton(...)
				...
				function SubButton:UpdateColors()
					SubButton.Label.TextColor3 = SubButton.Disabled and Library.DisabledAccentColor
						or Color3.new(1, 1, 1)
				end
		...
		function BaseGroupboxFuncs:AddInput(Idx, Info)
			...
			function Textbox:UpdateColors()
				...
				Library.RegistryMap[Box].Properties.TextColor3 = Textbox.Disabled and "DisabledAccentColor"
					or "FontColor"
			end
		...
		function BaseGroupboxFuncs:AddSlider(Idx, Info)
			...
			function Slider:UpdateColors()
				...
				HideBorderRight.BackgroundColor3 = Slider.Disabled and Library.DisabledAccentColor
				...
				Library.RegistryMap[HideBorderRight].Properties.BackgroundColor3 = Slider.Disabled
						and "DisabledAccentColor"
					or "AccentColor"
		...
		function BaseGroupboxFuncs:AddDropdown(Idx, Info)
			...
			function Dropdown:Display()
				...
				if Info.Multi then
					...
					local StringValue = if typeof(Dropdown.Value) == "Instance"
						then Dropdown.Value.Name
						else Dropdown.Value
					ItemList.Text = Info.FormatDisplayValue and tostring(Info.FormatDisplayValue(StringValue))
						or StringValue
				end
			end
		...
		function Library:Notify(...)
			...
			function Data:Resize()
				...
				pcall(
					...
					NotifyOuter,
					UDim2.new(0, XSize * DPIScale + 8 + 4, 0, YSize),
					"Out",
					"Quad",
					0.4,
					true
		...
		function Window:AddTab(Name)
			...
			function Tab:UpdateWarningBox(Info)
				...
				Library.RegistryMap[TopBarLabel].Properties.TextColor3 = Tab.WarningBox.IsNormal == true and "FontColor"
					or nil
				Library.RegistryMap[TopBarLabelStroke].Properties.Color = Tab.WarningBox.IsNormal == true and "Black"
					or nil
			end
