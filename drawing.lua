...
local genv = getgenv();
local genv = getgenv();
local pi = genv.math.pi;
local huge = genv.math.huge;
local _assert = clonefunction(genv.assert);
local _color3new = clonefunction(genv.Color3.new);
local _instancenew = clonefunction(genv.Instance.new);
local _mathatan2 = clonefunction(genv.math.atan2);
local _mathclamp = clonefunction(genv.math.clamp);
local _mathmax = clonefunction(genv.math.max);
local _setmetatable = clonefunction(genv.setmetatable);
local _stringformat = clonefunction(genv.string.format);
local _typeof = clonefunction(genv.typeof);
local _taskspawn = clonefunction(genv.task.spawn);
local _udimnew = clonefunction(genv.UDim.new);
local _udim2fromoffset = clonefunction(genv.UDim2.fromOffset);
local _udim2new = clonefunction(genv.UDim2.new);
local _vector2new = clonefunction(genv.Vector2.new);
